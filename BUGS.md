...
> **Last Updated:** 2026-09-17 (High Priority Security & Reliability Fixes Applied)
> **Scope:** Full codebase audit (backend, frontend, configuration)
> **Total Bugs Found:** 22
> **Status:** 20 fixed, 2 remain (see Status column below)
...
### Fixed (20)
- C1, C2, C3, C4, C5, C6, C7 (CRITICAL)
- H1 (false alarm), H2, H3, H4, H5, H6, H7 (HIGH)
- M2, M4, M5, M6, M7, M8 (MEDIUM)

### Remaining (2)
- M1: Duplicate auth middleware — refactor candidate
- M3: Frontend route protection — React Router migration candidate
...
- **File:** `frontend/src/services/api.ts:512`, `backend/src/routes/auth.routes.ts`
- **Code:** `res.cookie('hms_student_auth_token', token, { httpOnly: true, sameSite: 'lax', path: '/' })`
- **Status:** FIXED — Added `httpOnly` cookie support for student authentication while preserving Bearer header fallback.
- **Description:** Student authentication tokens were previously stored only in `localStorage`, making them accessible to any JavaScript executed by the page.
- **Fix:** Implemented `httpOnly`, `sameSite: 'lax'` cookie setting on login and clearing on logout across backend and frontend services.

### C6. Management SSE Token in URL Query String
- **File:** `frontend/src/services/api.ts`, `backend/src/routes/management.routes.ts`
- **Code:** `POST /api/management/events-stream/ticket`
- **Status:** FIXED — Implemented single-use short-lived (30s) SSE authentication tickets.
- **Description:** Management SSE (Server-Sent Events) connection previously embedded the long-lived JWT in the URL query parameters.
- **Fix:** Clients now request a 30-second single-use ticket via authenticated POST before establishing SSE `EventSource` connection.
...
- **Code:** `max: 15` (15 requests per 15-minute window per IP)
- **Status:** FIXED — Reduced rate limit to 15 login attempts per 15-minute window per IP.
- **Description:** 100 login attempts per 15 minutes was too generous for authentication endpoints.
- **Fix:** Reduced `max` to 15 attempts in `loginRateLimiter`.
...
- **Code:** `startHeartbeat()` and client registration lifecycle listeners
- **Status:** FIXED — Added active error/close listeners and response destruction checks in heartbeat pings.
- **Description:** SSE connections dropped silently at network level previously remained in connection maps.
- **Fix:** Implemented active pruning during heartbeat pings (`res.destroyed || res.writableEnded`) and attached `close`/`error`/`end` event listeners.
